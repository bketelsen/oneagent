...
export const readLogsTool = defineTool({
  ...
  handler: async ({ filePath, lines }) => {
    try {
      // TODO: consider streaming for large files — reading the entire file
      // into memory before slicing could cause memory pressure on very large
      // log files (hundreds of MB).
