      ...
      <Layout title={`Run ${run.id}`}>
        ...
        <div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-6 mb-8">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <div class="text-sm text-gray-500 dark:text-gray-400">Run ID</div>
            ...
            <div>
              <div class="text-sm text-gray-500 dark:text-gray-400">Started At</div>
              <div>{run.startedAt}</div>
            </div>
            <div>
              <div class="text-sm text-gray-500 dark:text-gray-400">Completed At</div>
            ...
            <div>
              <div class="text-sm text-gray-500 dark:text-gray-400">Retry Count</div>
        ...
        {events.length === 0
          ? <p class="text-gray-400 dark:text-gray-500">No events recorded for this run.</p>
          : <div class="space-y-2">
              {events.map((event) => (
                <div class="bg-gray-100 dark:bg-gray-800 rounded-lg p-4">
                  <div class="flex justify-between items-start mb-2">
                    <span class="text-blue-400 font-medium">{event.type}</span>
                    <span class="text-gray-400 dark:text-gray-500 text-sm">{event.ts}</span>
                  </div>
                  <pre class="text-sm text-gray-700 bg-gray-50 dark:text-gray-300 dark:bg-gray-900 rounded p-3 overflow-x-auto">
